...
import { colorGrey, colorNormal, setBorder, setFlowToRight, setMargin, setFlex, setFlexWidth, setPadding, media } from "../utils";
...
export const List = styled.ul`
  ...
  list-style: none;
  // flex & flex-direction
  ${({flexDirection}) => flexDirection && typeof flexDirection === 'string' ? setFlex({direction: flexDirection}) : null}
  ${({flexDirection}) => flexDirection && flexDirection.desktop ? setFlex({direction: flexDirection.desktop}) : null}
   ...
   // flex & justify-content & align-items
    ${({flexXY}) => flexXY && typeof flexXY === 'object' ? setFlex({x: flexXY.x, y: flexXY.y}) : null}
   ${({flexXY}) => flexXY && flexXY.desktop ? setFlex({x: flexXY.desktop.x, y: flexXY.desktop.y}) : null}
   ${({flexXY}) => flexXY &&flexXY.tabland ? media.tabland(setFlex({x: flexXY.tabland.x, y: flexXY.tabland.y}))  : null}
   ${({flexXY}) => flexXY &&flexXY.tabport ? media.tabport(setFlex({x: flexXY.tabport.x, y: flexXY.tabport.y})) : null}
   ${({flexXY}) => flexXY &&flexXY.phone ? media.phone(setFlex({x: flexXY.phone.x, y: flexXY.phone.y})) : null}
   // flex & align-items
...
const ListItem = styled.li`
  position: relative;
  ${({pd}) => pd&& typeof pd === 'object' ? setPadding({x: pd.x,
          y: pd.y,}) : null}
   ${({pd}) => pd&&pd[0]  ? setPadding({x: pd[0].x,
          y: pd[0].y,}) : null}
   ${({pd}) => pd&&pd[1] ? media.tabland(setPadding({x: pd[1].x,
          y: pd[1].y,}))  : null}
   ${({pd}) => pd&&pd[2] ? media.tabport(setPadding({x: pd[2].x,
          y: pd[2].y,})) : null}
   ${({pd}) => pd&&pd[3] ? media.phone(setPadding({x: pd[3].x,
          y: pd[3].y,})) : null}
  ${(props) =>
